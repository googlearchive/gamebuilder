	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				half2 timeoffset = half2(0, -_Time.x*2*_Speed);
				//get parallax values
				float heightval = tex2D(_NoiseTex, i.uv+ timeoffset);
				...
				float val = tex2D(_NoiseTex, i.uv+(offset)+timeoffset);
				float4 color = tex2D(_ColorTex, i.uv+(offset)+half2(_Time.x*5*_Speed,0));
				color = lerp(color,_ChangeColor,1-val);
				//sparkles
				fixed3 sparklemap = tex2D(_SparkleTex, (i.uv+offset2)*_Scale + timeoffset); 
				...
				rim = tex2D(_ColorTex,half2(rim,rim)).a;
				color += _RimColor * rim;
